...
public class EntityAttackState : BaseAttackState
{
    protected override void HandleAnimationEvent()
    {
        ...
        if (attackDataInstance.EffectsList != null && attackDataInstance.EffectsList.Count > 0)
            foreach (Effect effect in attackDataInstance.EffectsList)
                effect.Execute(initialCastPayload);
        
        // 3. Apply Recoil if necessary
        if (attackDataInstance.Animation == AnimationBool.IsAttackingStrong) 
            controller.EntityMover.ApplyRecoil(castDirection);
        
        // 4. Apply VFX
        if (attackDataInstance.Prefab != null)
        {
            ...
            GameObject spellVFX = Object.Instantiate(attackDataInstance.Prefab, spawnPosition, spawnRotation, controller.transform);
