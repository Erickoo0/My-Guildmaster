	...
	public class BaseAIEditor : EditorBase
	{
		...
		protected override void Inspector()
		{
			...
			if (isAIPath)
			{
				...
				var nacc = EditorGUILayout.Popup("Max Acceleration", acc, new[]
				{
					"Default", "Custom"
				});
			...
			if (canUseGravity)
			{
				...
				var ngrav = EditorGUILayout.Popup("Gravity", grav, new[]
				{
					"None", "Use Project Settings", "Custom"
				});
				if (EditorGUI.EndChangeCheck())
				{
			...
			{
				EditorGUI.BeginDisabledGroup(true);
				EditorGUILayout.Popup(new GUIContent(gravity.displayName, "Disabled because a non-kinematic rigidbody is attached"), 0, new[]
				{
					new GUIContent("Handled by Rigidbody")
				});
				EditorGUI.EndDisabledGroup();
			}
			...
			{
				EditorGUILayout.HelpBox("You are using both a Rigidbody and a Character ControllerEntity. Those components are not really designed for that. Please use only one of them.", MessageType.Warning);
			}
		}
	}
}
